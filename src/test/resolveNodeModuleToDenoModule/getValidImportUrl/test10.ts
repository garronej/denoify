...
(async () => {
    ...
    assert(inDepth.same(ModuleAddress.parse("https://deno.land/std/node/buffer.ts"), moduleAddress));

    assert(inDepth.same(ModuleAddress.parse("https://deno.land/std@master/node/buffer.ts"), { ...moduleAddress, "branch": "master" }));
