...
async function denoify(params) {
    ...
    }
    {
        ...
        (await resolvePathsWithWildcards_1.resolvePathsWithWildcards({
            ...
            .forEach(resolvedPath => fsCopy_1.fsCopy(resolvedPath, path.join(denoDistPath, resolvedPath)));
        objIncludes
            .forEach(({ src, destDir, destBasename }) => fsCopy_1.fsCopy(src, path.join(denoDistPath, path.join(destDir !== null && destDir !== void 0 ? destDir : path.dirname(src), destBasename !== null && destBasename !== void 0 ? destBasename : path.basename(src)))));
    }
}
