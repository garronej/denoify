...
function getDenoCompatibleImportFactory(params) {
    const getDenoModuleUrlFromNodeModuleNameStatic = makeStatic(params.getDenoModuleUrlFromNodeModuleName);
    return (importStr) => __awaiter(this, void 0, void 0, function* () {
        ...
        const { url, indexPath } = yield getDenoModuleUrlFromNodeModuleNameStatic(moduleName);
        return path.join(...[
            path.join(url, rest.length === 0 ? indexPath : ""),
            ...rest.map((...[, index]) => index === rest.length - 1 ? rest[index] + ".ts" : rest[index])
        ]);
    });
}
...
function replaceImports(params) {
    return __awaiter(this, void 0, void 0, function* () {
        const { sourceCode, getDenoModuleUrlFromNodeModuleName } = params;
        const getDenoCompatibleImport = getDenoCompatibleImportFactory({ getDenoModuleUrlFromNodeModuleName });
        let out = sourceCode;
        for (const quoteSymbol of [`"`, `'`]) {
            ...
            const replacerAsync = (() => {
                ...
                return (substring) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    return `${before}${yield getDenoCompatibleImport(importStr)}${after}`;
                });
            })();
