...
(async () => {
    {
        const input = ["import {", "    execute as defaultExecute,", "    getOperationAST,", '} from "graphql"'].join("\n");
    ...
    for (const target of ["{ webSocket, WebSocketSubjectConfig }", "* as rxjsWs"]) {
        ...
        assert(output === [`import ${target} from "https://cdn.skypack.dev/rxjs@6.6.2/webSocket?dts";`].join("\n"));
    }

    {
        ...
        assert(output === `import ipaddr from "https://jspm.dev/ipaddr.js@1.8.1"`);
    }
    {
        const output = await consumeExecutableReplacer({
