...
function resolveFactory(params) {
    ...
    async function resolve(params) {
        ...
        const wrap = await (async () => {
            ...
            const resolveResult = await Scheme_1.Scheme.resolveVersion(scheme, { "version": (_a = scheme.branch) !== null && _a !== void 0 ? _a : "master" });
        ...
        repository } = JSON.parse(fs.readFileSync(path.join(getTargetModulePath(nodeModuleName), // node_modules/js-yaml
        "package.json")).toString("utf8"));
        const result = await (async () => {
            if (!wrap.tryRepositoryField) {
                return undefined;
            }
            const repositoryUrl = repository === null || repository === void 0 ? void 0 : repository["url"];
            if (!repositoryUrl) {
                return undefined;
            }
