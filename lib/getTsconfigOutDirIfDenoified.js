...
exports.getTsconfigOutDirIfDenoified = (() => {
    ...
    async function getTsconfigOutDir(params) {
        const { scheme } = params;
        return {
            "tsconfigOutDir": path.normalize(commentJson.parse(await node_fetch_1.default(Scheme_1.Scheme.buildUrl(scheme, { "pathToFile": "tsconfig.json" })).then(res => res.text()))["compilerOptions"]["outDir"])
        };
    }
