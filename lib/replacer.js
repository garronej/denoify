...
function consumeExecutableReplacerFactory(params) {
    ...
    const consumeExecutableReplacer = addCache_1.addCache(async (params) => {
        ...
        try {
            return await st.exec(`${process.argv[0]} ${filePath} ${JSON.stringify(ParsedImportExportStatement_1.ParsedImportExportStatement.stringify(parsedImportExportStatement))} ${version} ${JSON.stringify(sourceFileDirPath)}`);
        }
        catch (error) {
